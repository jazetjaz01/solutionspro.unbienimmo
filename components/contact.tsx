import { MailIcon, MapPinIcon, PhoneIcon } from "lucide-react";
import Link from "next/link";

const Contact = () => (
  <div className="min-h-screen flex items-center justify-center">
    <div className="text-center">
      <b className=" font-sarina">
        unbienimmo
      </b>
      <h2 className="mt-3 text-2xl md:text-4xl font-semibold tracking-tight">
        Nous contacter
      </h2>
      <p className="mt-4 text-base sm:text-lg text-muted-foreground">
        Notre équipe est disponible pour vous répondre rapidement
      </p>
      <div className="max-w-(--breakpoint-xl) mx-auto py-24 grid md:grid-cols-2 lg:grid-cols-3 gap-16 md:gap-10 px-6 md:px-0">
        <div className="text-center flex flex-col items-center">
          <div className="h-12 w-12 flex items-center justify-center bg-primary/5 dark:bg-primary/10 border border-border/30 shadow-xl/2  text-primary rounded-full">
            <MailIcon />
          </div>
          <h3 className="mt-6 font-semibold text-xl">Email</h3>
          <p className="mt-2 text-muted-foreground">
            Notre service client vous répond par email
          </p>
          <Link
            className="mt-4 font-medium text-primary"
            href="mailto:contact@unbienimmo.com"
          >
            contact@unbienimmo.com
          </Link>
        </div>
        <div className="text-center flex flex-col items-center">
          <div className="h-12 w-12 flex items-center justify-center bg-primary/5 dark:bg-primary/10 border border-border/30 shadow-xl/2  text-primary rounded-full">
            <MapPinIcon />
          </div>
          <h3 className="mt-6 font-semibold text-xl">Où nous trouver</h3>
          <p className="mt-2 text-muted-foreground">
            Vous pouvez nous rendre visite
          </p>
          <Link
            className="mt-4 font-medium text-primary"
            href="https://map.google.com"
            target="_blank"
          >
             7 avenue de Banyuls sur mer <br /> 66100 Perpignan France
          </Link>
        </div>
        <div className="text-center flex flex-col items-center">
          <div className="h-12 w-12 flex items-center justify-center bg-primary/5 dark:bg-primary/10 border border-border/30 shadow-xl/2  text-primary rounded-full">
            <PhoneIcon />
          </div>
          <h3 className="mt-6 font-semibold text-xl">Téléphone</h3>
          <p className="mt-2 text-muted-foreground">Lundi au dimanche de 8h à 20h</p>
          <div
            className="mt-4 font-medium text-primary">
            0616224682
          </div>
        </div>
      </div>
    </div>
  </div>
);

export default Contact;
